...
from composer.rag.db import ChromaRAGDatabase
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Build ChromaDB RAG database from HTML documentation')
    parser.add_argument('html_file', help='Path to the HTML file to process')
    parser.add_argument('--output', '-o', required=True, help='Output directory for ChromaDB database')
    ...
    db = ChromaRAGDatabase(persist_dir=args.output, model=get_model(), skip_test=False)
    ...
    logger.info(f"ChromaDB database created at {args.output}")
