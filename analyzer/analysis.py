from typing import NotRequired, TypedDict, Iterator, cast
...
def _analyze_core(
    ...
    for (ty, d) in graph.stream(input=FlowInput(input=cast(list[str | dict], input_messages)), context=ExplainerContext(
