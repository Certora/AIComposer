...
def get_system_prompt() -> str:
    ...
    return load_jinja_template(config.system_prompt)

def get_initial_prompt(prompt: PromptParams) -> str:
    """Load and render the initial prompt from Jinja template"""
    return load_jinja_template(config.initial_prompt, **prompt)
...
def get_vfs_tools(
    ...
    if immutable:
        ...
        return vfs_tools(VFSToolConfig(
            fs_layer=fs_layer,
            immutable=False,
            forbidden_write=config.forbidden_write,
            put_doc_extra=config.put_doc_extra,
        ), AIComposerState)
...
def get_cryptostate_builder(
    ...
    crypto_tools: list[BaseTool] = ([solana_prover] if config.platform == "svm" else
        [certora_prover]) + [propose_spec_change, human_in_the_loop, code_result, cvl_manual_search(AIComposerContext), *vfs_tooling]
