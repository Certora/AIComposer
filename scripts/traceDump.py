...
from composer.workflow.services import get_checkpointer, store_context
...
async def main() -> None:
    ...
    async with store_context() as store:
        db = AuditStore(store)
        (run_info, vfs_init) = await db.get_run_info(thread_id=thread_id)

        # Initialize VFS from audit store
        ...
        async def handle_cvl_manual_search(step: dict, tool_id: str) -> SearchStep:
            """Handle cvl_manual_search tool case."""
            query = step["input"]["question"]
            assert isinstance(query, str)
            res_list: List[ManualResult] = []
            async for res in db.get_manual_results(
                thread_id=thread_id,
                tool_id=tool_id
            ):
                res_list.append(res)
        ...
        async def handle_certora_prover(step: dict, tool_id: str, queue: MessageQueue) -> ProverStep:
            ...
            results: list[RuleResult] = []
            async for r in db.get_rule_results(
                thread_id=thread_id,
                tool_id=tool_id
            ):
                results.append(r)
        ...
        async def parse_message(checkpoint: CheckpointTuple) -> list[Steps]:
            ...
            while prev is not None:
                ...
                summ = await db.get_summary_after_checkpoint(
                    thread_id=thread_id,
                    checkpoint_id=prev_id
                )
                if summ is not None:
                    prev_events = await parse_message(prev_tuple)
                        ...
                        case "tool_use":
                            ...
                            match which:
                                case "cvl_manual_search":
                                    events.append(await handle_cvl_manual_search(step, tool_id))
                                case "put_file":
                                    events.append(handle_put_file(step))
                                case "certora_prover":
                                    events.append(await handle_certora_prover(step, tool_id, queue))
        ...
        events.extend(await parse_message(x))
