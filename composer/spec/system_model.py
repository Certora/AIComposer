...
class FromSourceContract(ExplicitContract):
    """Base for contracts in the ``from-current-source`` workflow.

    Split into two concrete variants by relationship to the change being built:
    ``ExistingFromSource`` (contract is already in the source tree, either
    untouched or being edited) and ``FreshFromSource`` (new contract being
    introduced by this task — no source path yet).
    """
